...
export const makeApiCall = (
  ...
  // Retrieving access token is present in session storage
  const accessToken = window.sessionStorage.getItem("access-token");

  // Checking if it is present, then merge it with requestHeadersObj param
  if (accessToken !== null) {
    const authorization = `Bearer ${accessToken}`;

    // Merge authorization field with passed header object
    requestHeadersObj = { ...requestHeadersObj, authorization };
  }
